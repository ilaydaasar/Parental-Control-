...
DB_FILE = "parental_control.db"  # Veritabanı dosya adı
...
def is_time_exceeded(app_type, total_seconds):
    ...
    max_minutes = app_limits.get("duration", 0)  # ⚠️ NOT: "duration_seconds" değil "duration" olmalı
    action = app_limits.get("action", "none")
    if max_minutes > 0 and total_seconds > max_minutes * 60:
        return True, action
    return False, action

def start_limit_monitoring():
    # Gerekirse ayrı thread başlatmak için yer
    pass
